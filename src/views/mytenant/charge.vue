<template>
  ...
  <van-tabs v-model="active" type="line" color="#0084FF" title-active-color="#0084FF">
    <van-tab title="电表充值">
      ...
      <div class="chargecontent" :style="bodysize">
        ...
        <div class="chargerow3">
          ...
          <div class="chargerow-leftEPPannel">
            <meternumber totalwidth="100%" totalheight="60px" bgcolor="#0084FF" :bits="7" :value="meterleftEP" />
          </div>
          <div class="chargerow-colgap" />
        </div>
        <div class="chargerow4">
          <div class="chargerow-infopannel">
            <myinfo title="电价(kwh)" frcolor="#FFFFFF" :content="getPriceStr(price)" />
          </div>
          <div class="chargerow-infopannel">
            <myinfo title="累计电度(kwh)" frcolor="#FFFFFF" :content="getTotalEP()" />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    onChangeSelMeter(picker, value, index) {
      debugger
      if (value != null && value.meter != null) {
    ...
    loadMeter() {
      ...
      _this.$store.dispatch('GetTenantMetersByOpenid', this.user.openid).then((meters) => {
        _this.hideToast()
        if (meters == null || meters.lenght == 0) {
          return
        } else {
