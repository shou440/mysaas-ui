<template>
  <div class="paybody" :style="bodysize">
    ...
    <div class="payrow2">
      <div v-if="this.hasloadday" class="payrow2-monthpiker">
        <mytimeswiper :days="this.monthlist" @onChangeDayRange="onChangeDayRange" @onChangeDay="onChangeDay" />
      </div>
    </div>
    <div class="payrow-rowgap10" style="background-color:#eeeeee;" />
    <div class="payrow-rowgap10" style="background-color:#eeeeee;" />
    <div class="payrow3">
      <div v-for="bill in meterbills " :key="bill.fee_sn" class="meterbill-div">

        <meterbill
          :start_value_pro="bill.startvalue"
          :end_value_pro="bill.endvalue"
          :used_value_pro="bill.usedvalue"
          :price_pro="bill.price"
          :period_pro="bill.period_value"
          bill_title_pro="电费单"
          :weight_pro="bill.weight"
          :total_fee_pro="bill.totalfee"
          :bill_sn_pro="bill.sn"
          :room_name_pro="bill.roomname"
          :paystatus_pro="bill.paystatus"
          :bill_pro="bill"
          @selChange="onSelChange(bill)"
        />
      </div>
      <div v-for="bill in wmeterbills " :key="bill.fee_sn" class="meterbill-div">
    ...
    <div class="payrow4">
      ...
      <div class="payrow4-button">
        ...
        <van-button v-if="hasSelFee===true" ref="payButton" type="primary" size="large" @click="payFee();">支付(¥{{ totalFee.toFixed(2) }}元)</van-button>
        <van-button v-else ref="payButton" type="default" size="large" disabled>请选择收费单</van-button>
        <div class="payrow-buttongap" />
      </div>
      <div class="payrow-buttongap" />
    </div>
...
export default {
  ...
  created() {
    if (this.$store.state.mytenant.wxuser != null) {
  ...
  methods: {
    ...
    loadMeterFee(day) {
      ...
      var selDayMin = new Date(day.year, day.month - 1, 1, 0, 0, 0) // 按月的起始时间查询收费单
      var selDayMax = new Date(day.year, day.month - 1, 1, 23, 59, 59)
      var maxdate = new Date(selDayMin.getFullYear(), day.month - 1, 0).getDate()
    ...
    payFee() {
      ...
      for (i = 0; i < this.meterbills.length; i++) {
        ...
        if (bill != null) {
          var feeItem = { fee_sn: bill.sn, fee_type: 'MeterFee' }
          if (userid == 0) {
            userid = bill.user_id
          }
